...
export function Sidebar({ open, onClose }: { open: boolean; onClose?: () => void }) {
  ...
  return (
    ...
    <aside className={`${open ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-40 flex w-56 flex-col border-r border-brand-100 bg-white/95 shadow-xl backdrop-blur transition md:static md:translate-x-0 dark:border-red-950/45 dark:bg-[#09090b]/98 dark:shadow-black/50`}>
      <div
        onClick={() => goTo("/")}
        className="hover:cursor-pointer flex h-14 items-center border-b border-brand-100 px-2 dark:border-red-950/45 dark:bg-gradient-to-r dark:from-[#0b0b0d] dark:via-[#100708] dark:to-[#16090a]">
        <img
          src={logo}
          alt="Unique Carz"
        ...
        <div className="min-w-0">
          <div className="ml-2 text-base font-semibold text-brand-600 dark:text-brand-200">
            Unique Carz
          </div>
      ...
      <nav className="scrollbar-hidden flex-1 space-y-4 overflow-y-auto p-2 dark:bg-gradient-to-b dark:from-[#09090b] dark:via-[#0b0b0d] dark:to-[#0b0506]">
        {sections.map((section) => (
          <div key={section.label}>
            {section.label !== "Main" && (
              <div className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-wide text-brand-700/70 dark:text-red-200/55">
                {section.label}
              </div>
            )}
      ...
      <div className="sticky bottom-0 border-t border-brand-100 bg-white p-3 dark:border-red-950/40 dark:bg-[#09090b]">
